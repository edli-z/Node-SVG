...
const PlacementWorker = require("./placementworker")
const Parallel = require("paralleljs")
const _Promise = require("bluebird");
const { processNfpPair } = require ("./processNfpPair")
...
function SvgNest(){
	...
	this.start = function(progressCallback, displayCallback){						
		...
		function offsetTree(t, offset, offsetFunction){
			for(var i=0; i<t.length; i++){
				...
				if((typeof t[i].children === "function") && t[i].children().length > 0){
	...
	this.launchWorkers = function(tree, binPolygon, config, progressCallback, displayCallback){
		...
		*/
		...
		_Promise.map(nfpPairs, async (pair) => {

			return processNfpPair(pair, {
				searchEdges: config.exploreConcave,
				useHoles: config.useHoles
			})
		
		}, {concurrency: 4}).then(function(generatedNfp){
			...
			*/
			...
			_Promise.map([placelist.slice(0)], async (paths) => {

				return worker.placePaths(paths)
			
			}, {concurrency: 4}).then(function(placements){
	...
	this.applyPlacement = function(placement){
		...
		for(i=0; i<placement.length; i++){
			var newsvg = cloneDeep(svg);
			newsvg.clear()

			newsvg.attr('viewBox', '0 0 '+binBounds.width+' '+binBounds.height);
			newsvg.attr('width',binBounds.width + 'px');
			newsvg.attr('height',binBounds.height + 'px');

			var binclone = cloneDeep(bin);
			binclone.clear()
			
			binclone.attr('class','bin');
			binclone.attr('transform','translate('+(-binBounds.x)+' '+(-binBounds.y)+')');

			//newsvg.appendChild(binclone);

			binclone.addTo(newsvg)
			...
			for(j=0; j<placement[i].length; j++){
				...
				var partgroup = SVG('<g></g>');
				partgroup.attr('transform','translate('+p.x+' '+p.y+') rotate('+p.rotation+')');

				//partgroup.appendChild(clone[part.source]);
				
				clone[part.source].addTo(partgroup)
				...
				if(part.children && part.children.length > 0){
					...
					for(k=0; k<flattened.length; k++){
						...
						if(flattened[k].hole && (!c.attr('class') || c.attr('class').indexOf('hole') < 0)){
							c.attr('class',c.attr('class')+' hole');
						}
